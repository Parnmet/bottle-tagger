...
class MainWindow(ttk.Frame):
    ...
    def __init__(self, mainframe, path):
        ...
        Label(self.master, textvariable=self.folder_name, font=(None, 15)).grid(
            row=1, column=2
        )
        ...
        Checkbutton(
            self.master,
            text="default value",
            variable=self.check_default_value,
            command=self.onDefault,
        ).grid(row=3, column=1)

        self.tag_status = StringVar(value="Untagged")
        self.label_tag_status = Label(
            self.master,
            textvariable=self.tag_status,
            borderwidth=1,
            bg="red",
            font=(None, 15),
        )
        ...
        self.label_tag_value = Label(
            self.master, textvariable=self.tag_value, borderwidth=1, font=(None, 15)
        )
        ...
        Label(
            self.master, textvariable=self.action_value, borderwidth=1, font=(None, 20)
        ).grid(row=4, column=3)

        self.index_status = StringVar(value="1/" + str(len(self.path)))
        Label(self.master, textvariable=self.index_status, borderwidth=1).grid(
            row=6, column=3
        )
    ...
    def updateImageCanvas(self):
        ...
        self.canvas = CanvasImage(
            self.master, self.path[self.my_image_number]
        )  # create widget
        self.canvas.grid(row=0, column=1, columnspan=3, sticky="nsew")  # show widget
    ...
    def onBrowse(self):
        ...
        tempdir = filedialog.askdirectory(
            parent=self.master, initialdir=currdir, title="Please select a directory"
        )
        if len(tempdir) > 0:
    ...
    def onSubmit(self, event=None):
        ...
        # save to db
        db_manager.save(self.path[self.my_image_number], description, ijmetadata)
        if isChecked != 1:
