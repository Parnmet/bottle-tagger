...
class CanvasImage:
    ...
    def __init__(self, placeholder, path):
        ...
        # SCALE
        self.imscale = 0.7  # scale for the canvas image zoom, public for outer classes
...
class MainWindow(ttk.Frame):
    ...
    def __init__(self, mainframe, path):
        ...
        ttk.Frame.__init__(self, master=mainframe, style="BW.TLabel")

        self.master.title('Advanced Zoom v3.0')
        self.master.geometry('1920x1080')  # size of the main window
        ...
        # self.master.columnconfigure(2, weight=1)
        self.canvas = CanvasImage(self.master, './chang.png')  # create widget
        self.canvas.grid(row=0, column=1,columnspan=3, sticky="nsew")  # show widget

        self.folder_name = StringVar(value = "Please select a folder (ie. 41_broken_top)")
        Label(self.master, textvariable=self.folder_name,font=(None, 15)).grid(row=1, column=2)
        button_browse = Button(self.master, text ="Browse", command=self.onBrowse)
        ...
        # Tagged status label
        self.check_default_value = IntVar()
        Checkbutton(self.master, text="default value", variable=self.check_default_value, command=self.onDefault).grid(row=3, column=1)

        self.tag_status = StringVar(value = "Untagged")
        self.label_tag_status = Label(self.master, textvariable=self.tag_status, borderwidth=1,bg="red",font=(None, 15))
        self.label_tag_status.grid(row=2, column=3)
        ...
        button_previous.grid(row=3, column=1, sticky=E)
        button_next.grid(row=3, column=4, sticky=W)
        button_submit = Button(self.master, text ="Submit", command=self.onSubmit)
        button_submit.grid(row=7, column=2)

        self.master.bind('<Left>', self.onPrevious)
        self.master.bind('<Right>', self.onNext)
        self.master.bind('<Return>', self.onSubmit)
        self.master.bind('<KP_Enter>', self.onSubmit)
        

    def getImageInformation(self):
        self.isTagged = False
        frames = tifffile.TiffFile(self.path[self.my_image_number])
        page = frames.pages[0]
    ...
    def updateImageCanvas(self):
        ...
        self.canvas = CanvasImage(self.master, self.path[self.my_image_number])  # create widget
        self.canvas.grid(row=0, column=1,columnspan=3, sticky="nsew")  # show widget
        self.index_status.set(str(self.my_image_number+1) +"/"+str(len(self.path)) )

    
    def onBrowse(self):
        currdir = os.getcwd()
        tempdir = filedialog.askdirectory(parent=self.master, initialdir=currdir, title='Please select a directory')
        if len(tempdir) > 0:
            # print ("You chose %s" % tempdir)
            parent_folder = (tempdir.split('/'))[-1]
            self.default_tag_value = parent_folder.split('_')
            # self.e1.insert(END, default_tag_value[0])
            self.folder_name.set(parent_folder)
    ...
    def onPrevious(self, event=None):
        ...
        if self.my_image_number == 0:
            self.my_image_number = len(self.path)
        self.my_image_number -= 1

        # change image
        # self.canvas.destroy()
        self.updateImageCanvas()
        # self.index_status.set(str(self.my_image_number) +"/"+str(len(self.path)) )
        # self.e1.focus_set()
        self.master.after(200, lambda: self.e1.focus())
    ...
    def onNext(self, event=None):
        ...
        self.my_image_number += 1
        # return to first image
        if self.my_image_number == len(self.path):
            self.my_image_number = 0
        # change image
        # self.canvas.canvas.itemconfig(self.canvas.path, self.path[self.my_image_number])
        # self.canvas.path = self.path[self.my_image_number]
        # self.canvas.__imframe.update()
        # print(self.canvas.path)
        # self.index_status.set(str(self.my_image_number) +"/"+str(len(self.path)) )
        self.updateImageCanvas()
        # self.e1.focus_set()
        self.master.after(200, lambda: self.e1.focus())
    ...
    def onSubmit(self, event=None):
        ...
        #save to db 
        db_manager.save(self.path[self.my_image_number],self.e1.get())
        if isChecked != 1:
...
filename = ['test.jpg','test2.jpg','test3.jpg','test4.jpg','test5.jpg','test6.jpg','test7.jpg']  # place path to your image here
